...
```json
{ "error": "DEVICE_MISMATCH", "message": "This account is locked to another device." }
```
...
[
  {
    ...
    "active_trip": {
      ...
      "status": "active",
      "last_location": { "lat": 30.9010, "lng": 75.8573, "speed_kmh": 42, "recorded_at": "..." }
    }
  }
]
```
...
```json
{
  ...
  "stops": [{ "name": "Gate 2", "lat": 30.895, "lng": 75.850, "order": 1 }],
  "polyline": [{ "lat": 30.895, "lng": 75.850 }, { "lat": 30.897, "lng": 75.853 }]
}
```
**Response 201:**
```json
...
```json
[
  { "id": "uuid", "name": "Ravi Kumar", "email": "ravi@school.com", "device_id": "abc123" }
]
```
...
**Response 201:**
```json
{ "id": "uuid", "bus_id": "uuid", "route_id": "uuid", "driver_id": "uuid", "assigned_date": "2025-04-01" }
```
...
[
  {
    ...
    "started_at": "2025-04-01T08:00:00Z",
    "last_location": { "lat": 30.901, "lng": 75.857, "speed_kmh": 38, "recorded_at": "..." }
  }
]
```
...
const channel = supabase
  ...
  .on('postgres_changes', {
    ...
    updateETA(payload.new);         // uses calculateETA from @shieldtrack/utils
  })
  .subscribe();
```
...
### 6.2 ETA calculation (rolling 5-heartbeat average)

```ts
export const calculateETA = (
  current: { lat: number; lng: number },
  destination: { lat: number; lng: number },
  speedHistory: number[]          // km/h, last N readings
): number => {                    // returns minutes, Infinity if stopped
  const recent = speedHistory.slice(-5);
  const avgSpeed = recent.reduce((a, b) => a + b, 0) / recent.length;
  if (avgSpeed < 1) return Infinity;
  const distKm = haversine(current, destination) / 1000;
...
```ts
export interface Tenant   { id: string; name: string; institute_code: string; }
export interface User     { id: string; tenant_id: string; email?: string; role: 'admin'|'driver'|'parent'; device_id?: string; student_id?: string; }
export interface Bus      { id: string; tenant_id: string; plate_number: string; capacity: number; }
export interface Stop     { name: string; lat: number; lng: number; order: number; }
export interface Route    { id: string; tenant_id: string; name: string; polyline: {lat:number,lng:number}[]; stops: Stop[]; }
export interface Student  { id: string; tenant_id: string; name: string; route_id: string; }
export interface TripAssignment { id: string; bus_id: string; route_id: string; driver_id: string; assigned_date: string; }
export interface Trip     { id: string; tenant_id: string; assignment_id: string; bus_id: string; route_id: string; driver_id: string; status: 'active'|'completed'; started_at: string; ended_at?: string; }
export interface BusLocation { id: string; trip_id: string; bus_id: string; tenant_id: string; lat: number; lng: number; speed_kmh: number; recorded_at: string; }
export interface SOSEvent { id: string; trip_id: string; bus_id: string; tenant_id: string; lat: number; lng: number; triggered_at: string; resolved_at?: string; notes?: string; }
export interface DeviationAlert { id: string; trip_id: string; bus_id: string; tenant_id: string; lat: number; lng: number; distance_m: number; triggered_at: string; }
```
...
- [ ] Supabase Realtime enabled: `bus_locations`, `deviation_alerts`, `sos_events`

**Module 0.3 — Shared Packages** 🔴
- [ ] `packages/types/index.ts` — all interfaces
- [ ] `packages/utils/index.ts` — `haversine()` + `calculateETA()`
- [ ] Both importable as `@shieldtrack/types` and `@shieldtrack/utils`
...
- [ ] Uses `calculateETA` from `@shieldtrack/utils`
